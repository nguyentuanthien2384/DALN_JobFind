...
- `npm run test:image` chạy một container tạm, không mạng ngoài/DB thật/AI/SMTP, kiểm tra user không phải root, liveness, readiness thất bại đúng lúc, quyền `/status` và `/metrics`, giới hạn body, SIGTERM sạch.
...
Ngày 05-09-2026: 495 kiểm thử backend, 561 kiểm thử microservices, 18 kiểm tra tích hợp MySQL/RabbitMQ/HTTP trong container dùng một lần đã qua. Image Gateway đã build và qua bài kiểm thử cách ly. Cấu hình Compose hợp lệ. Các số này không chứng minh toàn hệ thống đạt SLO, chạy tải cao hoặc đã triển khai bản mới lên các container đang phục vụ người dùng.
