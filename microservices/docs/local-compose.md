...
- **Đợt 2n — Core thông báo sau duyệt:** creation/review mới lưu marker `approval-v1`, follower chỉ nhận intent sau quyết định PS1 được áp dụng; cùng template in-app như manual, không email follower. Tác giả vẫn có email/in-app nhưng câu chữ lịch sử, không khẳng định tin hiện còn công khai. Có **15 event**, HTTP không đổi, không DDL mới. Trong cửa sổ dừng ghi/nhận AI result và drain, cập nhật **tất cả Notification/handler/binding + Admin catalog trước tất cả Job Core writer/result-handler/relay 2n**; cập nhật catalog chung và giữ backend/frontend ít nhất 2m. Không trộn consumer/result-handler cũ với writer mới. Core cần SELECT request outbox, user/company/follower và INSERT outbox; giữ InnoDB/ledger như trước. Không dọn request `ai.moderate_job` còn có thể nhận kết quả, không đổi marker/ID/backlog. Backlog không marker giữ chính sách cũ; manual eligibility chưa đổi. Rollback dừng producer mới nhưng giữ consumer hiểu event/marker và request/outbox/inbox để đối chiếu. **2.657 test, 243 MySQL/22 ES/8 RabbitMQ tạm**, contracts, frontend build, build/test image local qua; image mới chưa thay container đang chạy. Chưa rollout/schema/dữ liệu thật; xem `client-sync.md` 2n. Các mốc dưới là lịch sử.

- **Đợt 2m — đăng lại có mã chống trùng:** dùng ledger và yêu cầu quyền/metadata của 2l, không DDL mới. Trong cửa sổ tạm dừng ghi, giữ **Notification → toàn bộ relay 2j → tất cả backend 2m → frontend 2m**, proxy giữ `Idempotency-Key`. Backend 2l trở về trước chưa xử lý mã đăng lại, không được chạy trộn hoặc nhận retry mới chưa rõ kết quả. Một mã/ý định chỉ tạo một bản và trừ một lượt; yêu cầu có key cần revision. Đối chiếu kiểm tra quyền actor/công ty/bản đã tạo hiện tại, không phụ thuộc nguồn còn tồn tại. AddPost giữ key/ngày đã gửi/revision trong sessionStorage cùng tab; nút đối chiếu gửi lại đúng payload, không tự POST/đổi mã, sửa sau từ chối vẫn giữ mã. Không giữ nháp sửa qua refresh hoặc bảo đảm tab độc lập/client không header. Rollback giữ ledger/receipt/outbox; không xóa key, purge backlog hoặc bật writer cũ để đối chiếu. **2.615 test, 222 nhóm MySQL tạm**, contracts/build frontend qua; không chạy lại ES/Rabbit/build image, không rollout/schema/dữ liệu thật. Xem `client-sync.md` 2m; các mốc dưới là lịch sử.

- **Đợt 2l — tạo tin có mã chống trùng:** giữ Notification/relay 2j và ledger `job_request_keys` từ 2c, kiểm tra InnoDB/PK `(userId, requestKey)`/`requestKey VARCHAR(128) ascii_bin`, quyền metadata/SELECT/INSERT/UPDATE trước rollout. Trong cửa sổ tạm dừng ghi, cập nhật **toàn bộ backend 2l trước frontend 2l** và đảm bảo proxy giữ `Idempotency-Key`; không chạy lẫn writer cũ bỏ qua mã. Backend không tự tạo/sửa ledger và không fallback khi sai schema. AddPost giữ mã/payload trong sessionStorage theo user/company, khôi phục qua reload cùng tab; đối chiếu chỉ theo nút bấm, không tự retry/đổi mã. Mã mới chỉ sau receipt đúng và bấm “Tạo tin khác”; 409/thiếu xác nhận giữ lại để đối chiếu. Đóng tab/mã riêng/client không header chưa được dedup. Đăng lại vẫn là 2k, chưa có idempotency. Rollback dừng client/writer mới và giữ ledger/outbox; không xóa key hoặc bật writer cũ để xử lý retry đang treo. **2.546 test, 194 nhóm MySQL tạm**, contracts/build frontend qua; không chạy lại ES/Rabbit/build image, không rollout/schema/dữ liệu thật. Xem `client-sync.md` 2l; các mốc dưới là lịch sử.

- Đợt 2k: `/api/create-reup-post` legacy ghi bản mới/charge/`job.created` cùng transaction và bỏ direct emit; kiểm tra lại quyền dưới khóa, chặn PS4/hạn mới sai, revision nếu gửi phải khớp (409). AddPost gửi revision; cửa sổ chờ kết quả, giữ nháp/ngày khi lỗi, không tự retry. Giữ response legacy HTTP 200/mã 1,2,-1 ngoài conflict; client truyền Date/ISO phải đổi timestamp. Cập nhật backend trước frontend và giữ đầy đủ relay **2j**/Notification/Search/Admin; không trộn writer cũ. 2.467 test, 169 nhóm MySQL/21 Elasticsearch/7 RabbitMQ tạm, contracts và frontend build qua. Chưa build lại image/rollout/schema/dữ liệu thật. Chưa có HTTP idempotency legacy: reload/nhiều tab/gửi POST khác vẫn có thể trừ thêm lượt. Xem `client-sync.md` 2k và thứ tự rollback; không purge backlog hay bật emit song song.

- Đợt 2j: `/api/create-new-post` cũng INSERT một `job.created` cùng quota/tin/chi tiết, không emit trực tiếp sau commit. Tạm dừng tạo/sửa/duyệt để cập nhật **Notification consumer → tất cả relay Job Core 2j → backend và frontend**; relay 2h/2i chưa đủ để giữ producer legacy cho event tạo mới. Notification bỏ qua creation legacy chưa PS1, dùng intent duyệt 2g; policy Core/backlog khác giữ nguyên. AddPost giữ nháp/khóa Lưu khi kết quả tạo không chắc chắn, nhưng chưa có idempotency legacy hay lưu cảnh báo qua refresh. Schema/catalog/endpoint không đổi, outbox InnoDB cùng DB/quyền/dữ liệu hợp contract vẫn bắt buộc; lỗi đã rollback giữ HTTP 200/mã 2, bất ngờ mã -1. 2.415 test, 150 nhóm MySQL tạm, 7 RabbitMQ tạm, contracts, build frontend và build/test image local qua; chưa rollout/restart/schema/dữ liệu thật. Giữ relay/consumer để xử lý pending khi rollback, không purge hoặc bật emit song song. Tiếp theo đăng lại legacy; chi tiết `client-sync.md` 2j.

- Đợt 2i: sửa tin legacy `/api/update-post` cũng ghi `job.updated` trong transaction; bỏ emit sau commit, giữ chính sách PS3, hạn mức và response/AddPost hiện tại. Lỗi outbox đã rollback trả HTTP 200/`errCode: 2`, lỗi bất ngờ HTTP 200/`errCode: -1`, conflict 409 (không dùng HTTP 503 của manual). Giữ điều kiện 2g–2h; cập nhật **tất cả relay hiểu `legacy-job` trước backend**, tạm dừng sửa/duyệt khi nâng cấp và đối chiếu pending khi rollback. Không sửa schema hay nới contract/dữ liệu lịch sử để vượt lỗi; không bật direct emit song song. 2.371 test, 134 nhóm MySQL tạm, 18 Elasticsearch tạm, 6 RabbitMQ tạm và contracts qua. Chưa rollout backend/container/schema thật, chưa build lại image/frontend trong đợt này. Chi tiết ở `client-sync.md` 2i; tiếp theo tạo/đăng lại tin legacy.

- Đợt 2h: backend lưu `job.updated` manual cùng quyết định/intent 2g trong outbox, bỏ emit sau commit. Không đổi schema/catalog/API. Giữ Search current-source/CAS và binding `job.updated`, bảo đảm API nội bộ Job Core đọc cùng MySQL; **cập nhật toàn bộ relay Job Core hiểu marker `aggregateType=legacy-job` trước backend** trong cửa sổ tạm dừng duyệt. Không trộn relay cũ gắn sai producer hoặc bật lại emit trực tiếp. Kiểm tra dữ liệu cũ hợp contract trên bản sao; payload không hợp lệ phải rollback, không tự sửa dữ liệu để duyệt tiếp. Dashboard hint vẫn best-effort nhưng lỗi hint không đổi phản hồi quyết định thành thất bại. 2.346 test, 122 nhóm MySQL tạm, 6 RabbitMQ tạm, 15 Elasticsearch tạm và image build/test qua; chưa rollout backend/container/schema thật. Giữ relay/consumer mới để đối chiếu pending khi rollback, không purge/recreate tài nguyên. Chi tiết ở `client-sync.md` 2h; tiếp theo là outbox sửa tin legacy.
