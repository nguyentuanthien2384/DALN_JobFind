...
1. **Phần hợp đồng còn lại:** HTTP của 5 service nghiệp vụ đã có OpenAPI/validator; còn schema payload/version compatibility của toàn bộ event RabbitMQ, consumer-driven contracts và các route monolith/Socket.IO. Chưa nghiệm thu mọi trang frontend hoặc mọi dữ liệu lịch sử. Kiểm tra kiểu/độ dài `fileBase64` không thay thế kiểm tra nội dung PDF an toàn.
