...
Đồng bộ client theo yêu cầu tiếp theo: đợt 1 xử lý phiên/lỗi và polling AI; đợt 2a đồng bộ hạn mức; đợt 2b đồng bộ sửa tin độc lập và giữ đúng ngày hết hạn. CI có test/build frontend và tích hợp các writer. Chưa chuyển API đăng tin hoặc tạo màn hình AI/CV; còn API đăng lại/idempotency, adapter dữ liệu form/Allcode, kiểm duyệt legacy và xử lý xung đột form trước khi chuyển UI. Xem `client-sync.md` về cách áp dụng, giới hạn và thứ tự tiếp tục.
