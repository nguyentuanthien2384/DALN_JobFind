...
Lệnh chạy lại, cách ly/cleanup và giới hạn ở `compose-background-acceptance.md`. Không cấu hình SMTP, không gọi provider trả phí, không đổi `.env`/schema/dữ liệu/container thật. Không coi readiness Identity/Application là nghiệm thu CV/Kanban, hoặc token/realtime fixture là login/Socket.IO thật. Giữ mọi điều kiện writer/ledger/fence/rollback 2q–2u. **Tiếp theo bước lớn 2: đồng bộ màn hình AI/CV và tìm kiếm/API legacy còn thiếu theo từng đợt.**
