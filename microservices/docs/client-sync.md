...
## Đợt 2o: đồng bộ điều kiện đăng lại, hạn tin và quyền công ty

Ngày 06-09-2026, thống nhất điều kiện tạo **bản đăng lại mới** giữa Job Core và backend legacy, không chuyển màn hình sang endpoint mới hoặc đổi chính sách AI/manual. Không thêm bảng, endpoint hay event: vẫn **50 thao tác HTTP, 15 event**.

### Quy tắc và tương thích

- Nguồn chỉ được đăng lại khi PS1/PS2/PS3, có timestamp mili giây nguyên dương hợp lệ và `timeEnd <= thời điểm kiểm tra`. Từ chối PS4/trạng thái không biết, ngày thiếu/hỏng, chuỗi có khoảng trắng/ký hiệu mũ/số thập phân. Không tự sửa dữ liệu lịch sử hoặc coi ngày hỏng là đã hết hạn.
- COMPANY/EMPLOYER/ADMIN đều phải thuộc **cùng công ty hiện tại với chủ tin**, công ty S1/CS1 và đủ lượt đúng loại nguồn. ADMIN không được sao chép nguồn của công ty khác bằng chức năng đăng lại; quyền quản trị đọc/sửa/duyệt ngoài công ty không thay đổi. Đây là thay đổi hành vi cố ý của backend legacy, kể cả client không gửi key: trước đây có thể đăng lại tin đang hạn và ADMIN có thể sao chép sang công ty khác.
- Kiểm tra lại membership, company, trạng thái/ngày của nguồn sau khóa user có thứ tự → company → post → detail. Hạn mới phải còn trong tương lai **sau khi chờ khóa detail**, trước trừ lượt/tạo bản mới. Khi bị từ chối, transaction không để lại key đang treo, bản sao, lượt bị trừ hay event. Bản mới vẫn PS3, giữ loại tin nguồn; Core sao chép detail và yêu cầu AI, legacy dùng snapshot detail bất biến và chờ manual như trước.
- `JobRepost` Core bổ sung `expectedRevision` tùy chọn (`jv1-` + 64 hex thường), đối chiếu source/detail dưới khóa. Phiên bản cũ trả HTTP 409/`conflict: true`; không tự đọc phiên bản mới rồi retry. Helper `repostJob(sourceId, timeEnd, {...options, expectedRevision})` gửi nguyên phiên bản đã giữ; giá trị gửi rõ nhưng sai bị chặn, không âm thầm bỏ đi. AddPost vẫn dùng legacy và đã gửi revision từ 2m.
- **Không gửi revision giữ nguyên hash v1 cũ**, không thêm giá trị mặc định vào ý định/receipt. Core vẫn nhận client cũ không revision, nên chưa có precondition cho mọi caller. Khi đã có receipt, thêm/bỏ/đổi revision dưới cùng key là ý định khác và bị 409. Không đổi operation, response snapshot, key, payload outbox hoặc receipt đã chấp nhận.
- Tra cứu receipt chạy trước kiểm tra nguồn mới: cùng key/payload trả lại bản đã tạo dù nguồn bị sửa, kéo dài hạn, ngày hỏng, gỡ hoặc không còn tồn tại; ngày đã gửi hết hạn vẫn được đối chiếu. Vẫn kiểm tra quyền công ty của actor và bản đã tạo hiện tại. Client không key không có bảo đảm chống tạo trùng; Core/legacy không dùng chung operation khi chuyển endpoint, không fallback để đối chiếu.
- AddPost chỉ hiện Đăng lại cho trạng thái hợp lệ đã hết hạn, kiểm tra lại trong nút xác nhận và giải thích tạo bản mới/trừ lượt trong cùng công ty. ADMIN vẫn không có nút đăng lại trên màn hình này. Cảnh báo/nút đối chiếu pending giữ riêng với điều kiện tạo mới: nguồn hết quyền/hỏng/đang hạn không làm mất key/payload cũ hoặc tự phát sinh mã mới.

### Áp dụng

Giữ các điều kiện ledger/outbox và thứ tự Notification/Admin/relay ở 2m–2n. Trong cửa sổ dừng ghi và drain, cập nhật **toàn bộ backend legacy và Gateway/Job Core 2o trước frontend/helper gửi revision mới**; không trộn Core cũ bỏ qua revision hoặc Gateway cũ từ chối trường mới. Kiểm tra dữ liệu và các client ngoài AddPost trên bản sao trước rollout vì điều kiện legacy đã siết. Không thêm DDL, backfill ngày, chuyển công ty, purge key hoặc xóa receipt để né lỗi. Khi rollback, dừng client/writer mới, giữ ledger và consumer tương thích để đối chiếu; không dùng writer cũ bỏ qua key/revision cho yêu cầu đang treo.

### Kiểm chứng

**2.697 test qua**: backend 741 (37 suite), microservices 1.033 (55 file), frontend 923 (53 suite). Contracts, build frontend và build/test image Docker local qua. **274 nhóm MySQL tạm**, gồm 31 nhóm mới dùng HTTP/writer/transaction thật cho Core, legacy có key và không key; kiểm tra ma trận role/status/loại tin, deadline/quyền sau lock wait, revision cũ, rollback, receipt cũ và 20 yêu cầu cùng key. Fixture cũ đã được đặt ngày hết hạn rõ ràng cho nguồn cần đăng lại; không tự sửa ngày trong helper gửi request. Đã sửa ba kỳ vọng/cách dựng fixture mới (đồng hồ giả, detail pointer nội bộ không lộ qua HTTP và số dư sau tạo nguồn), không nới quy tắc production để vượt test.

Không chạy lại Elasticsearch/RabbitMQ trong đợt này vì không đổi transport/consumer/payload; số 22/8 ở 2n là kết quả lịch sử. Chưa nghiệm thu đăng nhập và toàn stack E2E. **Không rollout/restart container phục vụ, migration hoặc đổi dữ liệu thật; không gọi AI/SMTP/PayPal.** Image local đã build, container ứng dụng chưa thay; chỉ dọn MySQL tạm do fixture sở hữu. Bước tiếp theo là đồng bộ điều kiện thông báo follower khi duyệt manual (hạn tin/công ty) trước khi chuyển thêm màn hình; chưa coi mọi nghiệp vụ Core–legacy đã đồng nhất.

## Đợt 2p: đồng bộ điều kiện thông báo follower khi duyệt manual

Ngày 07-09-2026, hoàn tất điều kiện **quảng bá tới người theo dõi**, không đổi quyền/quy tắc quyết định duyệt, API màn hình hoặc kênh thông báo. HTTP vẫn **50 thao tác**, catalog vẫn **15 event**, không thêm DDL.

### Hành vi và giao dịch

- Manual chỉ tạo follower intent khi quyết định `approve` thực sự đổi trạng thái, hạn tin là timestamp mili giây nguyên dương hợp lệ **lớn hơn thời điểm kiểm tra**, công ty hiện tại của tác giả S1/CS1. Ngày thiếu/hỏng/hết hạn hoặc công ty thiếu/bị khóa/chưa duyệt không quảng bá. Core `approval-v1` cũng chặn chuỗi ngày có khoảng trắng, ký hiệu mũ hay `.0`, tránh ép kiểu biến dữ liệu hỏng thành ngày hợp lệ; cùng quy tắc ngày chuẩn như 2o.
- Backend truyền deadline/trạng thái công ty từ **post/owner/company đã đọc dưới khóa**, không lấy các cờ do body gửi. Giữ thứ tự user có thứ tự → company → post → detail; kiểm tra giờ sau lock wait và các lần ghi quyết định, ngay trước truy vấn follower. Không đọc lại hoặc khóa company/user sau post. Manual giữ khóa company đến commit; Core giữ snapshot company/follower như 2n, không thêm khóa ngược thứ tự.
- Quyết định, Note, hủy request AI, `job.updated` và thông báo **tác giả** vẫn được lưu cùng transaction khi follower không đủ điều kiện. Đây không phải quy tắc cấm duyệt tin hết hạn/công ty bị khóa, cũng không khẳng định PS1 là đang hiển thị công khai. Từ chối/chặn/mở lại vẫn chỉ thông báo tác giả. Ngày trong nguồn đổi sau khi mở màn hình vẫn trả 409 theo revision trước mọi ghi.
- Chỉ truy vấn follower khi đủ điều kiện; danh sách người nhận được đóng băng một lần, dedup theo ID, chia nhóm 100. Tác giả đồng thời theo dõi công ty vẫn có hai audience riêng. Nếu truy vấn follower cần thiết hoặc ghi bất kỳ intent nào lỗi thì rollback cả quyết định; không bỏ qua lỗi để trả thành công hay gửi trực tiếp. Các trường eligibility chỉ là ngữ cảnh nội bộ, **không thêm vào payload event**.
- Consumer không tính lại điều kiện lúc nhận: thông báo đã lưu giữ nguyên người nhận/nội dung/ID dù sau đó tin hết hạn, công ty đổi trạng thái, tác giả đổi công ty hoặc người dùng bỏ theo dõi. Backlog manual trước 2p vẫn được xử lý như cũ, không purge/backfill/đổi ID. Core request không marker vẫn giữ chính sách lịch sử 2n. No-op hoặc replay không tạo thêm follower sau khi công ty được mở lại; một quyết định duyệt mới hợp lệ sau vòng chặn/mở lại là ý định mới, có thể tạo thông báo mới.
- Follower vẫn in-app/realtime, không email; tác giả giữ in-app/email và nội dung lịch sử. Giao diện ManagePost/chuông thông báo và HTTP response không đổi nên không cần sửa frontend. Việc dedup lưu inbox/delivery không phải bảo đảm exactly-once tại SMTP/realtime. Điều kiện được đánh giá tại thời điểm chọn người nhận, không phải cam kết tin còn hạn đến khi commit/gửi/đọc thông báo.

### Áp dụng và kiểm chứng

Giữ các điều kiện outbox/ledger và thứ tự Notification/Admin/relay từ 2n–2o. Trong cửa sổ dừng duyệt/nhận AI result và drain, cập nhật **toàn bộ backend legacy cùng Job Core result-handler 2p** để không chạy lẫn chính sách eligibility; backend phải triển khai đồng bộ writer và helper nội bộ. Không có binding/consumer schema mới, không cần đổi request frontend. Trước áp dụng kiểm tra dữ liệu cũ trên bản sao; giữ quyền đọc bảng follower, các bảng giao dịch InnoDB và bằng chứng policy request Core. Rollback phải dừng quyết định mới, giữ consumer tương thích và outbox/inbox/receipt hiện có để xử lý phần đã lưu; không tự xóa thông báo hoặc sửa ngày/công ty để ép điều kiện.

**2.739 test qua**: backend 774 (37 suite), microservices 1.042 (55 file), frontend 923 (53 suite). **306 nhóm MySQL tạm**, gồm 32 nhóm mới đối chiếu hai đường quyết định, ngày/công ty không hợp lệ, dữ liệu giả trong body, lock wait, revision cũ, rollback, mất phản hồi, cạnh tranh manual/AI, kênh tác giả/follower, replay và backlog cũ. HTTP/Sequelize/Core/consumer/inbox trong fixture là thật, nhưng danh tính/kết quả AI/dữ liệu là tổng hợp; không phải E2E đăng nhập toàn stack. Fixture cũ có kỳ vọng gửi follower đã được đặt rõ ngày tương lai và S1/CS1; thêm ca riêng cho ngày cũ/công ty không đủ điều kiện.

Contracts và build/test image local qua; không chạy lại Elasticsearch/RabbitMQ hoặc build frontend vì không đổi transport/consumer/payload/mã giao diện. Không cộng kết quả ES/Rabbit lịch sử vào số kiểm thử đợt này. **Chưa rollout/restart container phục vụ, migration, thay dữ liệu thật hoặc gọi AI/SMTP/PayPal.** Chỉ dọn MySQL tạm có nhãn sở hữu; image local mới chưa thay container đang chạy. Bước tiếp theo là thống nhất vòng đời sửa/duyệt AI–manual trước khi chuyển màn hình đăng tin sang Job Core, không coi mọi nghiệp vụ đã đồng nhất.

## Đợt 2q: mọi thay đổi thực sự đều phải duyệt lại

Ngày 07-09-2026, người dùng chọn **đưa mọi thay đổi về chờ duyệt, giữ manual ở luồng cũ và AI ở Job Core**. Đã đồng bộ quy tắc khi sửa; chưa chuyển endpoint màn hình. HTTP vẫn 50 thao tác, event vẫn 15 loại, không DDL/payload version mới.

### Quy tắc đã áp dụng

- Cả 11 trường nội dung/phân loại trong `DETAIL_FIELDS` (kể cả Markdown, lương, địa điểm, số lượng, giới tính) khi thực sự thay đổi đều làm tin PS1/PS2/PS3 về **PS3**. Job Core trước đây giữ trạng thái/yêu cầu AI khi chỉ sửa metadata; từ 2q phải tạo snapshot chi tiết và **request ID kiểm duyệt mới** cùng `job.updated`, trạng thái/fence và outbox trong transaction. Lỗi bất kỳ lần ghi nào rollback cả phiên bản mới và yêu cầu, không để lại phần đã ghi dở.
- Backend cũ vốn đã chuyển mọi sửa thực sự về manual PS3 và hủy request AI cũ; giữ nguyên nghiệp vụ, bổ sung kiểm thử từng trường. Job Core dùng AI cho mọi sửa thực sự, kể cả ADMIN sửa. Tác giả, hạn tin, loại tin và lượt đăng không đổi; không tự gia hạn hoặc chuyển công ty. PS4 vẫn không sửa được; mở lại thủ công chỉ trở về manual PS3, không tự gọi AI.
- So sánh dưới khóa sau xác minh revision. Giá trị không đổi sau chuẩn hóa (ví dụ amount số/chuỗi tương đương) là **no-op**: giữ trạng thái, revision, snapshot, thời gian và request, không có event/AI mới. No-op không tự đổi tin manual sang AI. Một sửa thực sự qua Core trên tin tạo bằng legacy có thể tạo request AI đầu tiên; loại xử lý theo writer được chọn, không suy ngầm từ PS3.
- Request ID mới loại kết quả cũ dù tiêu đề/HTML không đổi hoặc metadata A→B→A; không dùng riêng content hash để chấp nhận AI. Cạnh tranh với quyết định manual/AI kiểm tra dưới cùng khóa post/revision. Quyết định thủ công đã áp dụng hủy request hiện tại; kết quả cũ không được tự phục hồi PS3/PS4. No-op và xung đột không tự tạo request thay thế.
- **Phạm vi AI chưa mở rộng:** worker vẫn đánh giá tiêu đề và mô tả HTML như trước, không được hiểu đây là xác minh tính đúng đắn của lương/địa điểm/các danh mục. Mỗi sửa metadata thực sự ở Core nay có thể phát sinh thêm tác vụ và chi phí AI sau rollout, dù không trừ lượt đăng. Request cũ mất quyền ghi kết quả nhưng có thể vẫn được worker xử lý nếu đã xếp hàng/đang gọi provider; đợt này không triển khai hủy lệnh gọi đang chạy hoặc thay prompt.
- Search nhận `job.updated` PS3 và tiếp tục đọc nguồn hiện tại. Sự kiện duyệt cũ đến muộn không công khai metadata đang chờ duyệt; sau kết quả mới được áp dụng mới có thể hiển thị lại nếu đủ điều kiện công khai. Đồng bộ Search vẫn bất đồng bộ, không cam kết mất độ trễ. Thông báo đã lưu giữ nguyên snapshot theo 2n–2p.
- AddPost vẫn legacy/manual, có lời giải thích mọi thay đổi sẽ chờ duyệt. Giữ trạng thái khi response xác nhận no-op, cập nhật PS3 khi sửa thành công; giữ draft và không tự retry khi xung đột/kết quả chưa rõ. Chưa có HTTP idempotency riêng cho sửa: mất phản hồi phải đối chiếu dữ liệu/revision, không tạo mã mới hoặc bỏ revision để ép gửi lại.

### Áp dụng và kiểm chứng

Giữ điều kiện giao dịch/ledger/consumer/relay ở 2n–2p. Trong cửa sổ tạm dừng sửa và nhận AI result, drain phần đang xử lý rồi cập nhật **tất cả Job Core writer/result-handler 2q**, giữ backend ít nhất 2p và frontend tương thích. Không trộn Core cũ còn giữ quyết định khi sửa metadata. Không cần đổi binding, worker payload hoặc request client; cập nhật chính sách vận hành về số lần AI có thể tăng. Không backfill trạng thái/request cho tin đã sửa trước nâng cấp; không sửa/xóa queue, outbox, request ID, receipt hoặc inbox cũ. Rollback cần dừng sửa trước, giữ consumer/fence/ledger để đối chiếu; không bật writer cũ rồi coi dữ liệu mới vẫn được bảo vệ bởi quy tắc 2q.

**2.755 test qua**: backend 784 (37 suite), microservices 1.042 (55 file), frontend 929 (53 suite). **337 nhóm MySQL tạm** (31 nhóm mới: ma trận từng trường × PS1/PS2/PS3 × hai writer, no-op, generation ID, cạnh tranh, rollback, mất phản hồi và chuyển manual sang Core bằng sửa thực sự); **23 nhóm Elasticsearch tạm** (thêm một nhóm sửa metadata → PS3 → sự kiện duyệt cũ → duyệt mới). HTTP/writer/ORM và xử lý kết quả/inbox là thật trong fixture; AI/danh tính tổng hợp, HTTP nguồn Search mô phỏng. Không phải E2E đăng nhập/toàn stack hoặc kiểm chứng độ chính xác mô hình AI.

Contracts, build frontend, build/test image local qua; image test kiểm tra helper đóng gói cũng yêu cầu duyệt lại metadata nhưng bỏ qua no-op. Không chạy lại RabbitMQ trong đợt này vì không thay transport/schema payload. **Chưa rollout/restart container phục vụ, migration hoặc thay dữ liệu thật; không gọi AI/SMTP/PayPal.** Chỉ dọn MySQL/Elasticsearch tạm có nhãn sở hữu. Tiếp theo chuyển từng màn hình đăng tin sang Job Core bằng chế độ rõ ràng, giữ đường đối chiếu yêu cầu legacy đang treo và không tự chuyển mã/ý định giữa hai endpoint.

## Đợt 2r: tùy chọn tạo tin mới qua Job Core, giữ riêng ý định legacy

Ngày 07-09-2026. Phạm vi chỉ **tạo tin mới trên AddPost**, không chuyển API đọc/sửa/đăng lại hoặc quyết định manual. Không thay backend, DDL, HTTP/event contract (vẫn 50/15), ledger hay chính sách kiểm duyệt 2q.

### Lựa chọn luồng và khôi phục thao tác

- `REACT_APP_JOB_CREATE_MODE=legacy|core` là cấu hình **lúc build frontend**; không khai báo mặc định `legacy`. `frontend/.env.example` giữ `legacy` để chưa tự bật AI trên máy đang chạy. `core` chọn POST `/api/jobs` qua cùng Gateway; `legacy` chọn API tạo cũ. Sai giá trị chặn thao tác mới, không đoán hoặc fallback theo tình trạng dịch vụ. Đổi biến cần build/phục vụ lại frontend, không phải đổi biến trong container microservices.
- Giao diện ghi rõ manual hay AI. Core dùng `buildJobCreate` đã có để kiểm tra biểu mẫu, chuẩn hóa số lượng/danh mục, bỏ userId/companyId/role/status/revision; quyền vẫn do Gateway/backend kiểm tra. Ngày tương lai kiểm tra khi chuẩn bị tin mới/sửa sau từ chối; đối chiếu một yêu cầu pending gửi nguyên ngày cũ, kể cả đã quá hạn. Mã danh mục null đã lưu không bị thay bằng lựa chọn đầu khi tải lại.
- `jobfind:core-create:v1:<user>:<company>` lưu writer `core`, phiên bản, scope, mã ngẫu nhiên, payload và kết quả trong sessionStorage trước POST; xác minh đọc lại sau ghi. Giữ **nguyên namespace/định dạng legacy v1** và chỉ thêm writer ảo khi đọc, không chuyển/xóa/viết lại dữ liệu legacy để nâng cấp. Nội dung chưa bấm Lưu không được lưu bền qua refresh.
- Bất kỳ trạng thái legacy đã lưu (pending/rejected/blocked/succeeded) đều tiếp tục ở legacy, kể cả cấu hình mới là Core; ngược lại Core đã lưu không đổi về legacy khi hạ cờ. Mã/payload/writer đã lưu quyết định endpoint; cờ không được đọc lại để định tuyến retry. Không tự POST, fallback, gia hạn ngày hoặc đổi mã khi timeout, mất mạng, 401, 404/route chưa sẵn sàng, 5xx hay tải lại trang.
- Kiểm tra scope user/company trước chuẩn bị/gửi/xóa; hai bản lưu cùng scope ở hai luồng hoặc dữ liệu hỏng thì khóa và giữ bằng chứng để hỗ trợ đối chiếu, không tự chọn một bên. Chỉ receipt đã xác nhận và nút **Tạo tin khác** mới xóa đúng ý định hiện tại, rồi chọn cấu hình cho ý định mới. Stale view không xóa receipt của ý định khác. Hai tab mở độc lập vẫn là hai ý định, đóng tab/xóa storage/đổi origin không được bảo đảm khôi phục; không dùng các cách này để gỡ trạng thái chưa rõ.

### Phản hồi và giới hạn

- Core giữ HTTP 201/`{errCode:0,data:job}` và snapshot **lúc tạo được chấp nhận**, không thêm `replayed`, key server hoặc revision. Frontend đối chiếu ID, user/company, PS3 và toàn bộ trường đã gửi sau chuẩn hóa với snapshot. `idempotencyKey` helper hiện có thêm vào là **echo phía client**, không dùng làm bằng chứng server xác nhận. Phản hồi thành công thiếu/sai snapshot bị khóa, không báo thành công hoặc cho tạo mã mới. Việc này không thay thế ledger/quyền phía server.
- 400/403/409/413/415/422/429 đã xác định ở Core cho phép sửa rồi gửi lại **cùng mã, cùng writer**; 409 còn dùng cho hết quota nên không khóa vĩnh viễn mọi 409. Nếu mã đã có receipt cho nội dung khác, server tiếp tục 409, không tạo/trừ thêm lượt; cần hỗ trợ đối chiếu, không tự chuyển mã để vượt xung đột. Legacy giữ cách phân loại cũ (409/receipt thiếu khóa). Lỗi vận chuyển hoặc kết quả không đủ xác định giữ pending.
- Chặn bấm chồng trong một view; kết quả muộn vẫn đối chiếu và lưu đúng scope/ý định nhưng không thay biểu mẫu sau chuyển trang. Lỗi muộn không ghi đè Core receipt thành công hoặc cảnh báo malformed-success. Thay payload sau từ chối khiến phản hồi cũ không được ghi đè payload mới.
- Hạn mức được đọc lại theo **user và company** sau xác nhận; tạo thành công không đồng nghĩa AI đã duyệt hoặc tin đã công khai. Không lấy PS3 trong receipt làm trạng thái hiện tại. Nút xem tin vẫn tới màn hình legacy: **một sửa thực sự ở đó về manual PS3/hủy AI**, đăng lại vẫn legacy/manual. Giao diện thông báo rõ điều này. Màn hình chưa theo dõi tác vụ AI hoặc tự chuyển trang khi có kết quả.

### Áp dụng, rollback và kiểm chứng

1. Giữ các điều kiện 2l–2q: các writer cùng MySQL transactional/ledger, Core 2q và backend ít nhất 2p, Gateway mở route/header, relay/consumer Notification/Admin/Search hiểu chính sách 2n, schema và fence đúng. Không chạy lẫn backend bỏ qua key hoặc Core cũ bỏ qua duyệt lại metadata. Việc bật tạo Core có thể phát sinh tác vụ/chi phí AI sau rollout; AI vẫn đánh giá title/HTML.
2. Cập nhật các server/consumer cần thiết **trước** frontend; phục vụ frontend 2r ở `legacy` để giữ đường khôi phục. Sau nghiệm thu stack/quyền/hạn mức ở môi trường dự kiến, bật `core` và build/phục vụ frontend mới. Gateway đích phải cùng hệ thống/ledger, không chuyển base URL sang một database khác để đối chiếu mã cũ. Các yêu cầu legacy tồn tại trước khi bật vẫn đối chiếu legacy; phải duy trì endpoint đó.
3. Rollback chức năng tạo mới bằng **frontend 2r trở lên với cờ legacy**, không dùng frontend trước 2r vì nó không biết kho Core. Tiếp tục phục vụ Core và các consumer/fence/ledger để đối chiếu yêu cầu Core còn treo. Cờ chỉ ảnh hưởng ý định mới, không tắt được đường đối chiếu đã lưu. Không xóa storage/key/receipt, purge backlog hay dựng writer bỏ qua key. Hai kho cùng tồn tại cần đối chiếu có kiểm soát trước khi giải phóng, không có nút bỏ qua cảnh báo.

**2.860 test qua**: frontend 1.028 (55 suite), microservices 1.048 (56 file), backend 784 (37 suite). Có 99 test frontend mới cho lưu/khôi phục/định tuyến/receipt, validation, scope, lỗi, null classification và vòng đời view; 6 test dùng adapter + storage + helper frontend thật qua HTTP/validator/normalizer Core với receipt mô phỏng. Đây không phải E2E trình duyệt/Gateway đăng nhập hay lời gọi AI thật. Contracts và **337 nhóm MySQL tạm** (chạy lại bộ giao dịch/idempotency/manual–AI hiện có, không tăng số nhóm) qua; container MySQL sở hữu cùng dữ liệu dùng một lần đã được dọn và kiểm tra không còn. Frontend build **cả core và legacy** thành công; artifact cuối cùng trong `frontend/build` là legacy, chưa phục vụ bản mới.

Không đổi cấu hình thật, restart/rollout container đang phục vụ, migration/dữ liệu dự án thật hay gọi AI/SMTP/PayPal. Không chạy lại Elasticsearch/RabbitMQ hoặc build image microservices trong đợt giao diện này. Bước tiếp theo: **đọc và sửa tin qua Job Core bằng chế độ rõ ràng**, giữ revision/bản nháp và ranh giới manual–AI; sau đó mới chuyển đăng lại.

## Đợt 2s: đọc quản lý và sửa tin qua Job Core có kiểm soát

Ngày 07-09-2026. AddPost có thêm chế độ xem/sửa riêng. **Không chuyển đăng lại**, quyết định manual, danh sách/note hoặc bật cấu hình trên hệ thống đang phục vụ. Backend/DDL/HTTP/event contract không đổi (50 HTTP, 15 event); tiếp tục dùng nghiệp vụ 2q.

### Chọn luồng và kiểm tra dữ liệu

- `REACT_APP_JOB_EDIT_MODE=legacy|core` là cờ **lúc build frontend**, mặc định `legacy`, độc lập cờ tạo mới 2r. Sai giá trị chặn đọc/sửa mới. `core` dùng duy nhất GET `/api/jobs/:id/manage` và PUT `/api/jobs/:id`; không fallback sang public GET, Search hoặc backend cũ khi thiếu quyền, 404 hay gián đoạn. Mode được giữ theo lần mở biểu mẫu; tải lại có xác nhận trong cùng view không tự đổi writer theo cấu hình vừa thay.
- Giao diện nói rõ: Core đưa mọi sửa thực sự về AI PS3, kể cả tin trước đó manual. AI vẫn chỉ đọc title/HTML, không xác minh toàn bộ metadata. `legacy` giữ manual PS3. ADMIN vẫn chỉ xem trên màn hình này; không mở quyền sửa/đăng lại dù service Core có quyền API quản trị riêng. Tạo Core không tự quyết định mode sửa về sau; màn hình tạo đã cập nhật lời giải thích.
- Dữ liệu quản lý phải đủ trường, đúng ID và công ty hiện tại đối với employer/company; cho phép tin do đồng nghiệp cùng công ty đăng. Giữ tác giả, deadline, loại tin, mã danh mục lịch sử/null. Snapshot thiếu revision vẫn đọc được nhưng chặn Lưu; deadline hỏng hoặc PS4 không sửa/đăng lại. Không biến dữ liệu public/partial thành baseline có quyền ghi; phân quyền thực sự vẫn do Gateway/service và kiểm tra dưới khóa.
- `buildJobUpdate` chỉ gửi trường thực sự đổi cùng `expectedRevision` đã tải; không gửi userId/companyId/role/status hay toàn bộ trường chưa đổi. No-op tại biểu mẫu không gửi PUT/tạo tác vụ mới và không xác nhận trạng thái hiện tại trên server. Sửa nội dung, lương, số lượng, danh mục và Markdown đều về PS3 theo 2q. Deadline/isHot không sửa; không gia hạn/trừ lượt qua PUT.

### Giữ yêu cầu sửa chưa rõ kết quả

- Trước PUT Core, lưu `jobfind:core-edit:v1:<user>:<company>:<post>` trong sessionStorage, gồm baseline/owner/company, **nội dung đã gửi**, patch/revision và mã lần gửi cục bộ; đọc lại để kiểm tra lưu thành công. Mã này chỉ dùng so khớp thao tác trong trình duyệt, **không phải HTTP idempotency key**. Không thêm header idempotency cho PUT hoặc tự retry; revision vẫn là hàng rào cạnh tranh phía server.
- Bất kỳ lỗi PUT Core, 409, mất mạng/timeout, lỗi lưu xác nhận hoặc response sai đều giữ record và khóa lần gửi tiếp theo. Lỗi biểu mẫu phát hiện **trước** chuẩn bị record chỉ báo để sửa, chưa POST/PUT. Chặn click chồng và sửa biểu mẫu Core trong lúc chờ; kết quả muộn chỉ hoàn tất đúng record cũ, không xóa record mới/ghi đè view tin khác. Kiểm tra user/company/role trước gửi; response lưu thành công phải khớp tác giả/công ty, paid fields, mọi trường đã gửi, PS3 và revision mới.
- Refresh hoặc hạ cờ về legacy khi có Core record sẽ khôi phục đúng nội dung đã gửi và **giữ Core**, không tự GET/PUT để đoán kết quả. Cảnh báo còn giữ thì không tạo yêu cầu đăng lại mới. View legacy đã mở cũng kiểm tra record Core trước sửa; nếu thấy record mới thì chặn, tải lại sẽ đưa vào đối chiếu Core chứ không dùng legacy để vượt cảnh báo. Đường đối chiếu đăng lại legacy đã lưu từ 2m vẫn giữ nguyên endpoint/payload/key.
- **Tải lại tin → Bỏ phần chưa lưu và tải lại** là hành động do người dùng xác nhận sau khi sao chép nội dung cần giữ. Đọc lại private Core thành công, đúng scope/ID và có revision mới đọc được thì mới xóa đúng record đã xác nhận và nhận baseline hiện tại. Không đòi revision phải khác lần gửi: nếu GET còn thấy bản cũ do PUT đang chạy, lần ghi tiếp theo vẫn phải qua revision dưới khóa. Không xem GET này là receipt chứng minh PUT cũ đã thành công/thất bại, không tự áp dụng lại patch hoặc báo thành công cho thao tác cũ. GET lỗi/thiếu revision hoặc record đã đổi thì giữ draft/bằng chứng, không xóa trước khi đọc.
- Chỉ nội dung đã bấm Lưu được giữ qua refresh cùng tab; nháp chưa gửi hoặc phần gõ thêm sau lỗi chưa được autosave. Hai tab/thiết bị độc lập không chia sẻ record, storage không thay thế audit/backend, đóng tab/xóa storage/đổi origin không bảo đảm khôi phục. Record hỏng khóa cả đường Core lẫn đường legacy mới trong phạm vi này; không có nút bỏ qua bằng cách đổi mode. **PUT legacy vẫn chưa lưu nháp/yêu cầu bền qua refresh**; đây không phải mở rộng bảo đảm của Core cho mọi client cũ.

### Áp dụng và giới hạn vận hành

Giữ điều kiện 2q–2r: mọi Core writer/result-handler dùng chính sách 2q, backend ít nhất 2p, ledger/transaction/fence/outbox/consumer cùng hệ thống. Gateway mở route quản lý, giữ revision và không cho route mới rơi xuống legacy. Hoàn tất/đối chiếu các lần sửa legacy đang chạy trước khi đổi cấu hình: frontend trước 2s không có record PUT để khôi phục. Build/phục vụ frontend với cờ edit Core chỉ sau khi nghiệm thu stack/quyền; biến này không phải công tắc chặn mọi writer phía server hoặc bắt tab cũ tải bundle mới.

Rollback chức năng bằng **frontend 2s+ và cờ edit legacy**, giữ Core private read và writer/consumer/fence để hoàn tất phần đã nhận. Record Core chưa đối chiếu vẫn chọn Core; không rollback sang frontend trước 2s không hiểu kho sửa. Không xóa storage, ledger, request ID, snapshot hay backlog để gỡ lỗi. Cùng view vẫn giữ mode đã chọn cho tới khi rời/mở lại sau khi đối chiếu; không nới revision để ép lưu. Hạ cờ tạo mới 2r và cờ sửa 2s là hai việc độc lập.

Đăng lại vẫn `/api/create-reup-post`, kiểm tra hạn, quota và revision theo 2m–2o, giữ manual và mã cũ; chưa gọi Core repost. Danh sách/note và kiểm duyệt thủ công chưa chuyển; không có polling AI hoặc bảo đảm Search đã cập nhật ngay. Bước tiếp theo: **đăng lại qua Core**, giữ riêng tất cả ý định đăng lại legacy đang treo và không đổi nguồn/ngày/revision/key khi đối chiếu.

**2.959 test qua**: frontend 1.114 (57 suite), microservices 1.061 (57 file), backend 784 (37 suite). Có 86 test frontend mới cho snapshot/quyền/baseline/diff, lưu trước PUT, khôi phục qua refresh/hạ cờ, lỗi, cạnh tranh view, response muộn và bảo vệ đường legacy; 13 test nối helper frontend thật qua HTTP/validator cùng helper revision/chuẩn hóa Core cho 11 trường, xung đột và no-op. Những test HTTP này dùng dữ liệu/danh tính mô phỏng, không phải E2E đăng nhập/trình duyệt/AI.

Contracts và **337 nhóm MySQL tạm** hiện có qua (giao dịch/quota/idempotency/revision/manual–AI, không tăng số nhóm). Container MySQL sở hữu cùng dữ liệu dùng một lần đã được dọn; kiểm tra không còn container test. Frontend build với cả hai cờ tạo/sửa ở **core**, rồi cả hai ở **legacy**, đều thành công; artifact cuối trong `frontend/build` là legacy, chưa phục vụ bản mới. Không sửa cấu hình thật, restart/rollout container phục vụ, migration/dữ liệu dự án thật, hoặc gọi AI/SMTP/PayPal. Không chạy lại Elasticsearch/RabbitMQ hay build image microservices trong đợt giao diện này.

## Đợt 2t: đăng lại qua Job Core có lựa chọn riêng, giữ nguyên ý định legacy

Ngày 07-09-2026. Hoàn tất phần **chọn luồng đăng lại trên AddPost**, độc lập với tạo mới 2r và xem/sửa 2s. Không đổi backend, schema DB, 50 HTTP operation/15 event hoặc chính sách kiểm duyệt. Mặc định cả ba cờ vẫn legacy; chưa bật chức năng trên stack đang phục vụ.

### Luồng gửi và bản tin nguồn

- `REACT_APP_JOB_REPOST_MODE=legacy|core` được đọc khi mở màn hình, là biến **lúc build frontend**, không phải công tắc server. Không khai báo chọn legacy/manual; sai giá trị chặn ý định mới. Core gọi POST `/api/jobs/:id/repost` với đúng `{timeEnd, expectedRevision}` và `Idempotency-Key`, không gửi danh tính, status, loại tin hoặc nội dung từ biểu mẫu. Màn hình và hộp xác nhận ghi rõ AI hay thủ công. Xác nhận tạo thành công không có nghĩa đã duyệt/công khai.
- Trước một yêu cầu Core **mới hoặc sửa sau từ chối xác định**, đọc `/api/jobs/:id/manage`, kiểm tra snapshot đầy đủ, cùng công ty, PS1/PS2/PS3 đã hết hạn và revision **đúng phiên bản đang mở**. Nguồn cùng công ty do đồng nghiệp đăng được chấp nhận; tác giả tin mới là người đang thao tác. Không lấy bản nháp chưa lưu, không tự nâng revision/rebase khi tin đổi. Rủi ro thay đổi sau GET vẫn do precondition và khóa server xử lý. GET lỗi/sai phiên bản không lưu ý định hoặc POST, giữ biểu mẫu và cho tải lại có xác nhận.
- Khóa thao tác chồng từ trước GET đến hết POST; đổi trang/tài khoản trong lúc GET thì không tiếp tục ghi/gửi. Ngày tương lai kiểm tra lại sau GET. Lần đối chiếu **pending** bỏ qua hoàn toàn GET này, không đổi ngày/phiên bản dù tin gốc biến mất, đổi trạng thái, bản sửa Core đang chờ đối chiếu hoặc ngày đã gửi nay quá hạn. Quyền và việc replay/ghi mới vẫn do server quyết định.

### Khôi phục và xác nhận

- Kho riêng `jobfind:core-repost:v1:<user>:<company>:<source>` lưu trước POST: writer, scope, key 128-bit, payload, snapshot các trường tin nguồn được sao chép và trạng thái. Đọc lại xác minh ghi thành công. Giữ nguyên namespace/định dạng legacy v1, thêm writer ảo khi đọc, không sửa/xóa bản lưu để chuyển phiên bản. Snapshot chỉ chứa nội dung tuyển dụng cần đối chiếu, không token/CV. Nội dung chưa gửi trên biểu mẫu không được autosave.
- Mọi trạng thái đã lưu (pending/rejected/blocked/succeeded) giữ writer sau đổi cờ, kể cả cờ sai. Hai kho cùng tồn tại hoặc dữ liệu hỏng khóa đăng lại và giữ bằng chứng, không tự chọn, xóa, tạo mã mới hoặc gửi sang endpoint còn sống. Sau lỗi xác định chỉ sửa **cùng key/writer**; pending gửi lại đúng payload và snapshot cũ bằng nút đối chiếu. Không tự gửi khi tải trang. Ý định sửa Core chưa đối chiếu chặn đăng lại mới ở cả hai luồng, nhưng không chặn việc đối chiếu mã đăng lại đã gửi.
- Receipt Core là snapshot **lúc chấp nhận**: kiểm tra ID mới khác nguồn, tác giả/công ty, PS3, hạn mới, isHot và tất cả 11 trường sao chép kể cả giá trị null lịch sử. Không dùng key mà helper echo ở client làm bằng chứng server xác nhận; server hiện không trả `replayed`, key hoặc sourcePostId trong Core receipt. Sự liên kết với ý định vẫn dựa endpoint/mã/revision đã lưu và ledger phía server. Snapshot thiếu/sai bị blocked, không báo thành công hay tạo mã khác.
- Core 400/403/409/413/415/422/429 xác định cho sửa cùng mã; 409 có thể là quota hoặc revision/key conflict, cần đọc lại nguồn có xác nhận trước thử tiếp. Nếu mã đã được chấp nhận với payload khác, server tiếp tục từ chối, không trừ thêm lượt. 401/404/408/5xx/lỗi mạng hoặc kết quả chưa rõ giữ pending. Legacy giữ nguyên phân loại receipt/lỗi. Phản hồi muộn chỉ ghi đúng record ban đầu, không ghi đè payload mới hoặc màn hình khác; lỗi muộn không xóa receipt Core thành công/cảnh báo success sai.
- Thành công giữ tin gốc/nháp và nút **Xem tin đăng lại**, không tự chuyển trang. Giữ guard thành công của nguồn đó như legacy, chưa có nút bỏ receipt để đăng thêm từ cùng nguồn. Đóng tab, xóa storage, đổi origin hoặc tab/thiết bị khác không được bảo đảm khôi phục; không dùng chúng để giải phóng thao tác chưa rõ. GET nguồn không chứng minh kết quả của lần POST đã gửi.

### Áp dụng và bước kế tiếp

Giữ điều kiện writer/ledger/quota/revision/fence/consumer 2q–2s cùng hệ thống. Cập nhật server trước frontend, đối chiếu yêu cầu legacy và nghiệm thu quyền/nghiệp vụ trên stack dự kiến rồi mới bật cờ và build/phục vụ frontend. Core repost tạo tác vụ AI và có thể phát sinh chi phí provider; AI vẫn chỉ đánh giá tiêu đề/HTML. Không chuyển Gateway sang database khác để đối chiếu mã cũ.

Rollback dùng **frontend 2t trở lên với cờ repost legacy**, không về client không hiểu kho Core. Giữ cả hai endpoint, ledger/outbox/consumer để hoàn tất và đối chiếu phần đã nhận; không xóa storage/receipt/snapshot/backlog. Ba cờ độc lập, hạ cờ không thay writer của yêu cầu đã lưu, không thay bundle trong tab cũ hoặc chặn mọi writer server.

**3.091 test qua**: frontend 1.235 (59 suite), microservices 1.072 (58 file), backend 784 (37 suite). Có 121 test frontend mới (89 storage/receipt/source, 30 giao diện, 2 modal) và 11 test frontend qua HTTP/schema/helper revision thật với dữ liệu/danh tính mô phỏng. Các test này không phải E2E đăng nhập/trình duyệt/AI hoặc bằng chứng về ledger DB thật. Contracts qua, không thêm endpoint/event.

**337 nhóm MySQL tạm hiện có qua**, không tăng số nhóm: quota, transaction, revision, idempotency hai writer, snapshot, moderation và outbox với HTTP/ORM thật. Fixture đã xóa đúng container MySQL sở hữu cùng dữ liệu tổng hợp dùng một lần; kiểm tra không còn container mang nhãn test. Không dùng thông tin kết nối hoặc sửa database thật, không khởi chạy relay/provider trong bài này.

Frontend build với **cả ba cờ Core**, rồi **cả ba cờ legacy**, đều thành công; artifact cuối trong `frontend/build` là legacy. Kiểm tra lint các file logic/test mới và modal qua; không phát sinh thay đổi backend. Không sửa `.env` thật, phục vụ bundle mới, restart/rollout container, migration hay dữ liệu dự án, không gọi AI/SMTP/PayPal hoặc push Git. Không chạy lại Elasticsearch/RabbitMQ hoặc build image microservices trong đợt giao diện này. Đây chưa phải nghiệm thu E2E trên stack đang phục vụ.

Bước tiếp theo là **đồng bộ danh sách tin nhà tuyển dụng và thông tin kiểm duyệt với các luồng Core**, trước khi nghiệm thu xuyên màn hình và bật trên stack local. Không chuyển quyết định manual của quản trị sang AI; không cam kết Search/Notification đã cập nhật ngay khi POST thành công. Các mục PDF khác bên dưới vẫn chưa hoàn tất.

## Đợt 2u: danh sách riêng và thông tin kiểm duyệt nhà tuyển dụng qua Core

Ngày 08-09-2026. Bổ sung hai API chỉ đọc, adapter và hai màn hình ManagePost/NotePost. Không thay chính sách 2q: mọi sửa thực sự về PS3, legacy duyệt thủ công, Core dùng AI. Không thêm DDL/event hoặc đổi luồng ghi. Catalog tăng lên **52 HTTP operation (46 public/6 nội bộ), giữ 15 event**.

### Phạm vi dữ liệu và kiểm duyệt

- `GET /api/jobs/manage` chỉ cho COMPANY/EMPLOYER đọc tin công ty hiện tại, kể cả tin đồng nghiệp, chưa công khai, bị chặn và hết hạn. Lọc trạng thái, tìm theo tên/mã dạng chuỗi literal; không nhận companyId tùy chọn. Một câu SQL kiểm tra membership/công ty hoạt động/đã duyệt cùng count và trang, tránh đọc count ngoài phạm vi hoặc kiểm tra quyền tách rời dữ liệu. Sắp xếp updatedAt rồi ID giảm dần, giữ count khi trang rỗng/vượt cuối; mặc định 5, tối đa 50, offset tối đa 1.000.000. Đây là tính nhất quán trong từng lần đọc, **không phải snapshot cố định giữa các trang** hay chứng nhận hiệu năng dữ liệu lớn.
- `GET /api/jobs/:id/review` trả tóm tắt tin và các ghi chú thủ công cũ, phân trang theo createdAt/ID giảm dần; cùng một câu SQL kiểm tra quyền hiện tại, count và dữ liệu. Tin không tồn tại/ngoài công ty cùng 404 không kèm count; list ngoài phạm vi 403. Người ghi chú đã bị xóa không làm mất ghi chú. Trường lịch sử nullable/mã trạng thái lạ được hiển thị an toàn, không tự sửa DB hay suy diễn thành đã duyệt.
- `ai_requested` chỉ xác nhận có yêu cầu pending khớp tiêu đề/HTML hiện tại và PS3, không chứng minh worker đang xử lý. `ai_failed` là lỗi lần xử lý, không phải quyết định từ chối. `ai_applied` cần request applied, hash khớp và PS1/PS2. Cancelled là `no_active_ai`; thiếu/khác nội dung/trạng thái không nhất quán là `untracked`. Giữ tất cả writer/fence 2q trước khi dùng thông tin này; AI vẫn chỉ đánh giá tiêu đề/HTML, không xác minh mọi metadata.
- Response chỉ chọn trường cần hiển thị, không đưa request ID/hash/prompt/kết quả AI thô/HTML/CV lên UI. Notes là **lịch sử thủ công**, không phải lý do hoặc đầy đủ lịch sử AI. Không có note không đồng nghĩa chưa kiểm duyệt. Không thêm bảng audit AI, tự gọi lại AI, polling hay endpoint quyết định kiểm duyệt.

### Chọn luồng giao diện và xử lý lỗi

- Cờ build thứ tư `REACT_APP_JOB_WORKSPACE_MODE=legacy|core` độc lập tạo/sửa/đăng lại; không khai báo mặc định legacy. Cả bốn cờ trong `.env.example` vẫn legacy, `.env` thật không đổi. Mode được giữ khi mở view; sai cấu hình chặn đọc mới thay vì tự chọn dịch vụ còn sống. **ADMIN luôn legacy/manual**, kể cả cờ sai, và hai API Core mới cũng từ chối ADMIN. Quyền đọc chi tiết `/jobs/:id/manage` trước đây không đổi.
- Core hiển thị danh sách/tóm tắt tại lúc tải, có nút tải lại, giữ bộ lọc/phân trang và nút về trang đầu nếu trang rỗng. Xóa dữ liệu cũ khi bắt đầu đọc, lỗi hiện rõ thay vì biến thành danh sách rỗng. Kiểm tra danh tính trước/sau đọc và scope/shape/count của response; bỏ kết quả muộn sau khi đổi view/đổi tin. Không khẳng định tự xóa nội dung đang hiển thị ngay khi tài khoản ở tab khác đổi mà chưa có lần đọc/remount.
- Lỗi quyền/404/thiếu route/5xx không fallback sang public/Search/legacy, không tự POST/PUT hoặc gọi provider. Gateway không cho route mới rơi xuống legacy và đặt `Cache-Control: private, no-store`, kể cả lỗi xác thực. Frontend chỉ render note dạng văn bản. Chuyển sang sửa vẫn tải private detail riêng để lấy revision, không bịa revision từ list.
- Trạng thái hiện tại trong list/review **không là receipt** của lần tạo/đăng lại đang đối chiếu; không dùng nó xóa record sửa hay đổi writer/key/revision/ngày/payload đã lưu ở 2r–2t. Đã duyệt không bảo đảm còn hạn hoặc Search/Notification đã cập nhật.

### Áp dụng, rollback và kiểm chứng

Trước khi bật cờ: giữ điều kiện writer/ledger/transaction/consumer 2q–2t, kiểm tra các bảng hiện có `users`, `companies`, `posts`, `detailposts`, `notes`, `job_moderation_state` và quyền SELECT trên bản sao. Hai API không tự tạo/backfill bảng; bảng bắt buộc thiếu trả lỗi an toàn. Cần dữ liệu InnoDB để bảo đảm consistent read. Cập nhật **Job Core 2u → Gateway 2u → frontend 2u**, nghiệm thu quyền/vòng đời trên stack local cách ly trước khi build/phục vụ frontend với cờ workspace Core. Không đổi cấu hình Gateway sang database khác để đọc/đối chiếu ý định cũ.

Rollback tính năng dùng **frontend 2u+ và cờ workspace legacy**; ba cờ còn lại độc lập. Giữ các endpoint/ledger/fence/consumer 2r–2t để hoàn tất và đối chiếu yêu cầu đã nhận, không xóa storage/snapshot/receipt/backlog hoặc về client không hiểu kho Core. Hạ cờ không thay bundle đang mở hoặc khóa writer phía server.

**3.223 test qua**: frontend 1.314 (61 suite), microservices 1.125 (59 file), backend 784 (37 suite). Đợt này thêm 79 frontend (51 service/28 UI), 31 controller và 22 HTTP contract, bao gồm helper frontend thật. Kiểm tra lint các file mới/hai trang và contracts qua. Đây là kiểm thử tự động với dữ liệu/danh tính mô phỏng, chưa là E2E đăng nhập/trình duyệt/AI thật.

**348 nhóm tích hợp MySQL tạm qua**, thêm 11 nhóm workspace: scope/quyền thay đổi, PS1–PS4/hết hạn, tìm literal, count/trang vượt cuối, notes/author đã xóa, request Core → kết quả AI mô phỏng → manual → sửa → AI lỗi, dữ liệu lệch và bảng bắt buộc thiếu. Dùng HTTP/ORM thật, đọc không thay quota/outbox; AI result là dữ liệu tổng hợp, không gọi provider. Test tự dọn đúng container sở hữu cùng dữ liệu dùng một lần, không sửa database dự án.

Build frontend với **cả bốn cờ Core**, rồi **cả bốn cờ legacy**, đều qua; artifact cuối là legacy. Build/test image microservices local qua, gồm route mới yêu cầu xác thực/no-store, controller đóng gói và shutdown SIGTERM. Image/bundle mới **chưa thay bản đang phục vụ**. Không sửa `.env` thật, migration/dữ liệu thật, restart/rollout container, gọi AI/SMTP/PayPal hoặc push Git. Không chạy lại Elasticsearch/RabbitMQ vì không thay event/transport trong đợt chỉ đọc này.

Bước tiếp theo: **nghiệm thu xuyên màn hình và vai trò trên stack local cách ly**, nối tạo → danh sách → sửa → đăng lại → thông tin kiểm duyệt, trước khi bật các cờ cho môi trường đang dùng. Các giới hạn kiến trúc/vận hành của PDF vẫn theo `implementation-progress.md`, không coi hoàn tất nhóm giao diện là hoàn tất toàn bộ PDF.

## Đợt 2v: kiểm thử trình duyệt xuyên màn hình, giữ các bảo đảm 2a–2u

Ngày 08-09-2026. Đọc lại nhật ký từ các nền tảng outbox/inbox, bảo mật, HTTP/event contract đến 2a–2u và đối chiếu controller/service/storage hiện tại trước mở rộng kiểm chứng. Đợt này **chỉ bổ sung bộ kiểm thử và CI**, không thay nghiệp vụ/schema production hoặc chuyển endpoint/cờ thật. Giữ 52 HTTP/15 event, manual ở legacy và AI ở Core theo lựa chọn người dùng.

`npm run test:job-browser:integration` chạy 348 nhóm MySQL hiện có rồi thêm **12 nhóm browser integration**. Ba component AddPost/ManagePost/NotePost, modal, router/RouteGuard, Axios, lưu ý định và adapter đều là mã thật; Gateway chạy nguyên app trong tiến trình con, JWT/current identity/Redis/HTTP proxy thật. Core/controller/contract và backend controller/service/Sequelize chạy trên MySQL tạm. Không mock helper gửi tin hoặc chế receipt thành công. Chi tiết kiến trúc fixture/cách chạy/phần mô phỏng ở `job-browser-acceptance.md`.

- Chuỗi tạo → danh sách → ghi chú → editor; trạng thái hiện tại không thay receipt đã chấp nhận. Sửa metadata sau AI approval về PS3, giữ quota/tác giả/hạn; no-op không ghi và kết quả AI cũ bị chặn.
- ADMIN vẫn đọc list legacy và gửi quyết định/ghi chú thủ công; nhà tuyển dụng đọc trạng thái cancelled/note qua Core. Đăng lại dùng snapshot đã lưu, không dùng nháp; chỉ trừ một lượt, tạo bản mới riêng và chuyển đúng tin khi người dùng bấm xem.
- Mất phản hồi **sau commit thật** ở tạo/sửa/đăng lại: đi qua workspace rồi tải bundle legacy không mất ý định Core hoặc đổi mã/ngày/revision/payload. Tạo/đăng lại chỉ replay khi bấm đối chiếu và không thêm charge/record/outbox; PUT không tự replay, chỉ private GET có xác nhận để đối chiếu. Chiều ngược lại, legacy create đã gửi vẫn replay legacy khi mở bundle Core.
- Candidate bị chặn ở RouteGuard/API; khác công ty/ADMIN workspace/header giả không lấy được review riêng tư. Membership hiện tại được kiểm tra ở Gateway; response thuộc công ty mới không lọt vào view công ty cũ. Token hết hạn chuyển về trang login giữ chỗ và giữ nguyên record pending thật. Lỗi bảng đọc Core tạm khiến Gateway/UI báo lỗi, xóa note cũ, không fallback; tải lại sau phục hồi đọc đúng.

**Kết quả: 3.223 test hồi quy vẫn qua** (1.314 frontend/1.125 microservices/784 backend), **360 nhóm tích hợp qua** (348 cũ + 12 browser, Chromium). Không cộng 12 vào tổng unit test để tránh đếm trùng. Kiểm tra contracts, syntax, lint entry mới và YAML CI qua. Frontend production build với bốn cờ legacy qua, cùng tên/hash bundle như trước; test entry ngoài `src`, không lọt vào sản phẩm. Hai bundle test Core/legacy được build riêng trong thư mục tạm, không phải hai production build mới.

Đã thêm Playwright ghim 1.62.1 chỉ ở devDependencies/lockfile và bước CI cài Chromium/chạy browser integration sau dependencies frontend; chưa push hoặc chạy workflow GitHub. Lệnh MySQL cũ không cần browser/Redis. Browser dùng context tạm và chặn request ngoài origin fixture; Gateway con không kế thừa provider key/URL thật. Test dọn đúng MySQL/Redis sở hữu; bundle tạm của lần chạy thành công được dọn. Khi hoàn thiện fixture có một lần callback kết thúc sớm khiến tài nguyên thử còn lại; container sở hữu đã được kiểm tra/dọn, callback/cleanup được sửa và lần chạy lại hoàn tất sạch. Một thư mục bundle lỗi cũ `C:\Users\Admin\AppData\Local\Temp\jobfind-browser-SmIBF9` còn lại vì thao tác dọn thủ công bị môi trường chặn; chỉ là JavaScript thử tái tạo được, không có dữ liệu thật hoặc secret.

**Giới hạn:** auth session được cấp bằng token tổng hợp, trang login là placeholder; không thử form login hoặc toàn bộ App/HomeAdmin shell. Route legacy fixture dùng Gateway auth để gắn identity, không phải kiểm chứng toàn bộ auth legacy. AI result tổng hợp đi vào handler thật, không chạy provider/worker/relay/Search/Notification/SMTP/PayPal. Không coi đây là E2E Compose đầy đủ, hiệu năng hoặc chứng nhận hoàn tất PDF. Không thay `.env`, schema/dữ liệu thật, container đang phục vụ, gọi provider, build lại image runtime hoặc push Git.

**Bước tiếp theo:** ráp/nghiệm thu cấu hình Compose local cách ly và chuỗi xử lý nền bằng provider giả, trước khi bật các cờ trên môi trường đang dùng. Giữ thứ tự Core → Gateway → frontend 2u và tất cả điều kiện writer/ledger/consumer 2q–2t; kết quả test không cho phép bỏ sao lưu, đối chiếu pending hoặc hạ writer về phiên bản không hiểu mã/revision. Các hạng mục PDF còn lại tiếp tục theo `implementation-progress.md`.

## Thứ tự các đợt còn lại

1. **Ráp/nghiệm thu cấu hình Compose local cách ly và chuỗi xử lý nền trước khi bật cờ**. Tạo → danh sách → sửa → đăng lại → thông tin kiểm duyệt đã có browser integration 2v; chưa bao phủ form login/App shell hoặc queue → worker/provider → Search/Notification. Dùng provider giả trước, không tự gọi AI trả phí/SMTP. Tạo 2r, xem/sửa 2s, đăng lại 2t và workspace 2u mặc định vẫn legacy. Giữ writer/key/revision/ngày của yêu cầu đã lưu; review chưa bao gồm lý do/lịch sử AI đầy đủ.
