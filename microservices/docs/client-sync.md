...
1. **Đồng bộ danh sách tin nhà tuyển dụng và thông tin kiểm duyệt với Core**, giữ truy cập private/đúng công ty và phân biệt trạng thái hiện tại với receipt lúc tạo. AddPost đã có tùy chọn tạo 2r, xem/sửa 2s và đăng lại 2t; mặc định/stack đang phục vụ chưa bật Core. Giữ đối chiếu ý định cũ theo writer đã lưu; không đổi ngày/revision/key của lần đăng lại đang treo. Manual ở luồng cũ, AI ở Core chỉ đánh giá tiêu đề/HTML, không đồng nghĩa xác minh mọi metadata.
2. Nghiệm thu vòng đời xuyên màn hình và các client chưa gửi key/revision trước chuyển cờ trên stack local. Hạn mức, snapshot, idempotency, precondition và hàng rào manual–AI đã có kiểm thử riêng; không thay thế kiểm thử quyền và dữ liệu lịch sử trong môi trường dự kiến. Dashboard Socket.IO và publisher còn lại vẫn best-effort; không đồng nghĩa mọi event legacy đã bền. Giữ các giới hạn/rollback nêu trên; không chỉ đổi đường dẫn API.
