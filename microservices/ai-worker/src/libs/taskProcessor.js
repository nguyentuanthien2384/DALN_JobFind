...
// Provider/SDK exception text is untrusted. It can contain a proxy HTML body,
// request details or credentials and AI_RESULT is visible outside this worker.
const publicAiError = (error) => {
    // These codes are generated by our own contract validator, not the SDK.
    ...
    return 'Dịch vụ AI chưa thể xử lý yêu cầu';
};
    ...
    const execute = async (payload, handler, identity, payloadVersion) => {
        ...
        if (!isConfigured()) {
            ...
            try {
                ...
                logger?.error('goi AI that bai; khong tu goi lai', {
                ...
                data = { ...base, ok: false, error: publicAiError(error) };
            }
        }
