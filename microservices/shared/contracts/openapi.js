    ...
    for (const op of selected) {
        ...
        (paths[path(publicApi ? publicPath(op) : op.path)] ||= {})[op.method] = {
            ...
            description: [
                ...
                op.id === 'jobUpdate' ? 'New editors must submit expectedRevision from jobManageGet.editRevision. Server checks it under post/detail locks before writes and before no-op detection; mismatch returns 409 with conflict=true and no write/events. Success returns data.editRevision for the accepted state. Optional only for compatibility with old clients, which remain unprotected. This body precondition is not HTTP If-Match or an idempotency key; never retry by silently refreshing the revision.' : '',
                op.id === 'jobManageGet' ? 'Private current job read, including PS1/PS2/PS3/PS4. Non-admin access is restricted to the current approved company, rechecked with the job in one database statement; out-of-scope and missing IDs both return 404. ADMIN may inspect other companies. Returns raw classification codes and an opaque editRevision for guarded updates, not Allcode labels or AI internals. Missing/null revision means the client must not submit a guarded edit. PS3 means awaiting review, not proof a worker is processing. Cache-Control: private, no-store. Does not change the public jobGet visibility policy.' : '',
                ['jobManageList', 'jobReviewGet'].includes(op.id) ? 'Private, no-store recruiter workspace for COMPANY/EMPLOYER only; ADMIN continues using the separate legacy administration endpoints. Current approved company membership, rows and counts use one SQL statement/snapshot. No companyId selector, public/Search/legacy fallback or writes. Pagination limit defaults to 5 (1..50), offset to 0 (0..1000000); stable timestamp/id ordering per request, not a snapshot cursor across pages.' : '',
                op.id === 'jobManageList' ? 'Includes non-public and expired posts, minimal author identity, no description or editRevision. Search is literal title/ID substring (%, _ are not wildcards). Missing/stale company membership returns 403. Empty/out-of-range pages retain the scoped count. Load private detail before editing.' : '',
                op.id === 'jobReviewGet' ? 'Returns current post state and paginated historical manual notes. Missing/out-of-scope post returns the same 404. reviewState summarizes the current AI request with a matching content fingerprint; ai_requested is not proof of worker execution; ai_failed is not a rejection. No request IDs, hashes, prompts or raw AI results/reasons. Manual notes are not the full AI audit history.' : '',
                op.searchWindow ? 'offset + effective limit must not exceed 10000; limit=0 uses the default of 12. salaryJobCode, categoryJoblevelCode, categoryWorktypeCode and experienceJobCode accept repeated query keys (1..20 values, OR within each field, AND across fields). Scalar values remain supported. Count is exact; pages are limited to the first 10000 hits.' : '',
