...
const eventLabels = {
  ...
  identity_linked: 'Đã liên kết tài khoản ngoài', identity_unlinked: 'Đã hủy liên kết tài khoản ngoài',
  ...
  account_security_changed: 'Đã thay đổi mật khẩu hoặc trạng thái tài khoản', sso_rejected: 'Đăng nhập liên kết không thành công',
};
        ...
        <ul className="list-unstyled">
          {data.sessions.map(session => <li key={session.familyId} className="border rounded p-3 mb-3 d-flex flex-wrap justify-content-between align-items-center">
            <div><strong>{session.current ? 'Phiên hiện tại' : 'Phiên khác'} · {session.method === 'password' ? 'Mật khẩu' : providerLabels[(session.method || '').replace(/^oidc:/, '')] || 'Tài khoản liên kết'}</strong>
              ...
              <p className="mb-1">Hết hạn: {date(session.expiresAt)}</p>{session.rememberMe && <small>Đã chọn ghi nhớ đăng nhập</small>}</div>
