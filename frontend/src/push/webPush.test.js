...
const key = "AQID",
  storageKey = "jobfind:push-device";
...
beforeEach(() => {
  localStorage.clear();
  localStorage.setItem("token_user", "current-token");
  owner = null;
  ...
  registration = {
    active: {
      postMessage: jest.fn((data, ports) => {
        const changed =
          data.expectedOwner === undefined || data.expectedOwner === owner;
        if (changed) owner = data.ownerId;
        ports[0].reply({ ok: true, changed });
      }),
    },
    pushManager: {
...
test("explicit enable persists only device identity and sends authenticated browser subscription", async () => {
  expect(await status(7)).toBe("disabled");
  expect(Notification.requestPermission).not.toHaveBeenCalled();
  await enable(7, key);
  expect(owner).toBe(7);
  expect(await status(7)).toBe("enabled");
  ...
  await getConfig();
  expect(fetch.mock.calls[1][0].searchParams.get("id")).toBe("a".repeat(64));
});
test("denied or dismissed permission never registers or saves a device", async () => {
  for (const permission of ["denied", "default"]) {
    Notification.requestPermission.mockResolvedValueOnce(permission);
    await expect(enable(7, key)).rejects.toThrow();
  }
...
test("failed server registration unsubscribes the browser and clears worker ownership", async () => {
  ...
  await expect(enable(7, key)).rejects.toThrow("offline");
...
test("session change during registration rolls back using the captured account token", async () => {
  fetch.mockImplementationOnce(async () => {
    localStorage.setItem("token_user", "new-token");
    return response({ id: "a".repeat(64) });
  });
...
test("session change while permission is open cannot create a subscription", async () => {
  Notification.requestPermission.mockImplementationOnce(async () => {
    localStorage.setItem("token_user", "new-token");
    return "granted";
  });
  await expect(enable(7, key)).rejects.toThrow("Phiên đăng nhập");
...
test("expired local preference and a different account are not reported enabled", async () => {
  await enable(7, key);
  expect(await status(8)).toBe("disabled");
  const previous = JSON.parse(localStorage.getItem(storageKey));
  localStorage.setItem(
    storageKey,
    JSON.stringify({ ...previous, expiresAt: new Date(0) }),
  );
  expect(await status(7)).toBe("disabled");
});
...
test("startup detects stale push ownership after account changes", async () => {
  await enable(7, key);
  localStorage.setItem("userData", JSON.stringify({ id: 8 }));
  reconcilePushSession();
  await disable();
  expect(owner).toBeNull();
  expect(localStorage.getItem(storageKey)).toBeNull();
...
test("cross-tab Web Lock covers the full subscription update", async () => {
  ...
  Object.defineProperty(navigator, "locks", {
    configurable: true,
    value: {
      request: jest.fn(async (name, action) => {
        expect(name).toBe("jobfind-push-device");
        inside = true;
        try {
          return await action();
        } finally {
...
test("worker update failure still invalidates its account cache before logout", async () => {
  await enable(7, key);
  const put = jest.fn(),
    close = jest.fn();
  window.caches = {
    open: jest
      .fn()
      .mockResolvedValue({ match: async () => ({ json: async () => 7 }), put }),
  };
  ...
  registration.getNotifications = jest.fn().mockResolvedValue([{ close }]);
  registration.active.postMessage.mockImplementation((data, ports) =>
    ports[0].reply({ ok: false }),
  );
  try {
    await disable();
    expect(put).toHaveBeenCalledWith(
      "/__jobfind_push_owner",
