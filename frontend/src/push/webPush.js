...
const locked = (action) => {
  if (navigator.locks?.request)
    return navigator.locks.request("jobfind-push-device", action);
  const task = pending.catch(() => {}).then(action);
  pending = task;
  return task;
};
...
const api = async (
  ...
  try {
    const response = await fetch(
      new URL(path, new URL(URL_API, window.location.origin)),
      {
        method,
        headers: {
    ...
    if (!response.ok || result.errCode !== 0)
      throw new Error(
        result.code === "DEVICE_LIMITED"
          ? "Tài khoản đã bật thông báo trên 10 thiết bị. Hãy tắt ở thiết bị cũ."
          : "Không lưu được thiết lập thông báo. Vui lòng thử lại.",
      );
    return result.data;
  } finally {
    clearTimeout(timer);
...
export const getConfig = () =>
  api(
    "GET",
    `/api/push/config${device()?.id ? "?id=" + encodeURIComponent(device().id) : ""}`,
  );
const device = () => {
  try {
...
const activeWorker = async (registration) => {
  ...
  await new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      reject(new Error("Khởi tạo thông báo quá lâu."));
    }, 8000);
    const check = () => {
...
const setOwner = async (registration, ownerId, expectedOwner) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      channel.port1.close();
      reject(new Error("Không cập nhật được thiết bị thông báo."));
    }, 5000);
    channel.port1.onmessage = (event) => {
      ...
      event.data?.ok
        ...
        : reject(new Error("Không lưu được thiết lập thiết bị."));
    };
    worker.postMessage({ type: "push-owner", ownerId, expectedOwner }, [
      channel.port2,
    ]);
  });
};
const clearOwner = async (registration, expectedOwner) => {
  try {
    return await setOwner(registration, null, expectedOwner);
  } catch (error) {
...
export const enable = async (userId, publicKey) => {
  ...
  if (permission !== "granted")
    throw new Error(
      permission === "denied"
        ? "Thông báo đã bị chặn. Bạn có thể mở quyền trong cài đặt trình duyệt."
  ...
  return locked(async () => {
    ...
    try {
      ...
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ userId, id: saved.id, expiresAt: saved.expiresAt }),
      );
    ...
    if (registration) {
      ...
      if (changed) {
        ...
        if (subscription && !(await subscription.unsubscribe()))
          throw new Error(
            "Chưa tắt được thông báo trên thiết bị. Hãy thử lại.",
          );
      }
    }
...
export const clearPushOnLogout = () =>
  device() ? disable().catch(() => {}) : undefined;
...
export const reconcilePushSession = () => {
  ...
  if (
    ...
    (!localStorage.getItem("token_user") ||
      Number(user?.id) !== device().userId)
  )
    clearPushOnLogout();
};
window.addEventListener("storage", (event) => {
  if (event.key === "token_user" && event.oldValue !== event.newValue)
    clearPushOnLogout();
});
