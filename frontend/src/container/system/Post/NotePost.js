...
const NotePost = () => {
    ...
    const [loading, setLoading] = useListLoading(JSON.stringify([id, page, refresh]));
    useEffect(() => {
        let active = true;
        ...
        const load = async () => {
            ...
            } catch (failure) { if (active) { setNotes([]); setCount(0); setJob(null); setError(failure.message || 'Không đọc được thông tin kiểm duyệt'); } }
            finally { if (active) setLoading(false); }
        };
        load();
        return () => { active = false; };
    }, [id, page, refresh, user, workspace, setQuery, setLoading]);
    return <div className="col-12 grid-margin"><div className="card"><div className="card-body">
        ...
        {!error && <p>Số ghi chú thủ công: {count}</p>}
        <StableList busy={loading} resetKey={id}><div className="table-responsive pt-2"><table className="table table-bordered">
        ...
        {!error && count > 0 && <ReactPaginate forcePage={Math.min(page, Math.ceil(count / PAGINATION.pagerow) - 1)} previousLabel="Quay lại" nextLabel="Tiếp" breakLabel="..."
