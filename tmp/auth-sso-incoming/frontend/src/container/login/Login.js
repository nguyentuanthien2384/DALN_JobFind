...
import { establishSession, refreshSession, startGoogleLogin } from '../../auth/authClient';
...
const Login = () => {
    ...
    useEffect(() => {
        ...
        refreshSession().then((result) => {
            ...
            window.location.replace(['ADMIN', 'COMPANY', 'EMPLOYER'].includes(result.user.roleCode) ? '/admin/' : '/');
