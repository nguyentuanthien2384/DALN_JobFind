...
let handleBanUser = async (req, res) => {
    try {
        ...
        if (data.errCode === 0) {
            await authSessions.revokeAll(Number(req.body.data.id));
...
let handleChangePassword = async (req, res) => {
    try {
        ...
        if (data.errCode === 0) {
            await authSessions.revokeAll(req.user.id);
...
let changePaswordByPhone = async (req, res) => {
    ...
    let data = await userService.changePaswordByPhone(req.body);
        if (data.errCode === 0) {
            const account = await db.Account.findOne({ where: { phonenumber: req.body.phonenumber }, attributes: ['userId'] });
            if (account) await authSessions.revokeAll(account.userId);
            authSessions.clearRefreshCookie(res);
        }
        return res.status(200).json(data);
    } catch (error) {
