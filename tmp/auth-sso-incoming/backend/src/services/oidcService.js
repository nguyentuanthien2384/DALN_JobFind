...
import { hashOpaque, loadUser } from './authSessionService';
...
const providerSettings = (name) => {
  ...
  if (!issuer || !id || !secret || !redirect || !issuer.startsWith('https://') || !redirect.startsWith(process.env.NODE_ENV === 'production' ? 'https://' : 'http')) throw new Error('OIDC_CONFIG');
...
export const begin = async (name, res, linkUserId = null) => {
  ...
  await db.OidcTransaction.create({
    ...
    nonce, verifier, linkUserId, expiresAt: new Date(Date.now() + 5 * 60 * 1000),
  });
...
export const complete = async (name, req, res) => {
  ...
  const tx = await db.sequelize.transaction(async transaction => {
    const stored = await db.OidcTransaction.findByPk(hashOpaque(state), { transaction, lock: transaction.LOCK.UPDATE });
  ...
  let identity = await db.AuthIdentity.findOne({ where: { issuer: claims.iss, subject: claims.sub } });
  if (tx.linkUserId) {
    const current = await loadUser(tx.linkUserId);
    if (!current || (identity && identity.userId !== current.id)) throw new Error('OIDC_LINK_DENIED');
    if (!identity) identity = await db.AuthIdentity.create({
  ...
  return { userId: user.id, method: `oidc:${name}` };
};
