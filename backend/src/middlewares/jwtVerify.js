...
const middlewareControllers = {
    verifyTokenUser: (req, res, next) => {
        ...
        if (token) {
            ...
            jwt.verify(accessToken, getJwtSecret(), getJwtVerifyOptions(), async (err, payload) => {
                ...
                try {
                    if (!await validAccessSession(payload)) return res.status(401).json({
                        status: false, errMessage: 'Session revoked', refresh: true
                    });
                    req.auth = { sid: payload.sid || null, sub: Number(payload.sub) };
    ...
    verifyTokenOptional: (req, res, next) => {
        ...
        jwt.verify(accessToken, getJwtSecret(), getJwtVerifyOptions(), async (err, payload) => {
            ...
            try {
                if (!await validAccessSession(payload)) return next();
                req.auth = { sid: payload.sid || null, sub: Number(payload.sub) };
                const user = await db.User.findOne({
                    where: { id: payload.sub },
                    attributes: ['id', 'companyId'],
