...
const generate = (subject, output) => {
  if (!subject || !output || /[\r\n]/.test(subject))
    throw new Error(
      "Provide --subject=mailto:your-contact@example.com and --out=path",
    );
...
if (require.main === module) {
  try {
    ...
    console.log(
      "Saved Web Push configuration to " +
        generate(value("subject"), value("out")),
    );
  } catch {
    console.error(
      "Could not create keys. Supply a valid contact and a NEW output file; existing files are preserved.",
    );
    process.exitCode = 1;
  }
}
