...
test.each([
  ...
  expect(config.validSubscription({ ...valid(), endpoint })).toBe(false),
);
test("accepts real curve keys only, rejects extra identity fields and expired subscriptions", () => {
  const subscription = valid();
  expect(config.validSubscription(subscription)).toBe(true);
  expect(config.validSubscription({ ...subscription, userId: 999 })).toBe(
    false,
  );
  expect(config.validSubscription({ ...subscription, expirationTime: 1 })).toBe(
    false,
  );
  expect(
    config.validSubscription({
      ...subscription,
      keys: {
        ...subscription.keys,
  ...
  expect(
    config.validSubscription({
      ...subscription,
      keys: { ...subscription.keys, auth: "tiny" },
    }),
  ).toBe(false);
  expect(config.endpointId(subscription.endpoint)).toMatch(/^[a-f0-9]{64}$/);
});
test('allows Microsoft WNS provider subdomains without accepting suffix impersonation',()=>{
  expect(config.validEndpoint('https://wns2-test.notify.windows.com/w/?token=test')).toBe(true);
  expect(config.validEndpoint('https://wns2-test.notify.windows.com.evil.test/w/')).toBe(false);
  expect(config.validEndpoint('https://evilnotify.windows.com/w/')).toBe(false);
});
test("disabled by default, validates VAPID without exposing private keys", () => {
  ...
  try {
    ...
    process.env.WEB_PUSH_PRIVATE_KEY = "sensitive-invalid";
    ...
    process.env.WEB_PUSH_SUBJECT = "mailto:tests@example.com";
    expect(config.settings()).toEqual(
      expect.objectContaining({ timeout: 5000, TTL: 3600 }),
    );
  } finally {
    process.env = env;
  }
